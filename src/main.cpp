...
void showWindow() {
    ...
    const int numCircles = 50000;
    const int gridSize = 32;

    Circle* circles = Utils::generateCirclesAoS(numCircles, width, height, gridSize);

    ParallelRendererOverPixels renderer(width, height, numCircles);
...
void benchmark() {
    ...
    std::vector<int> gridSizes = {1, 4, 8, 16, 32};
    ...
    for (int numCircles : circleCounts) {
        for (int gridSize : gridSizes) {
            ...
            if (gridSize == 1) {
                // Con griglia 1x1 ha senso solo le versioni sequenziali e over pixels
                for (int numThreads : threadCounts) {
                    omp_set_num_threads(numThreads);
                    std::cout << "\n--- Test Over Pixels con " << numThreads << " Threads (GridSize 1x1) ---" << std::endl;

                    double avgParOverPixels = 0, avgParSoAOverPixels = 0;
